 ...
 * @param {string}  opts.accion              - 'CREACION' | 'MODIFICACION' | 'BAJA' | 'LOGIN' | 'CAMBIO_CONTRASENA' | 'CUENTA_BLOQUEADA'
 ...
 * @param {string}  [opts.usuario_rol]       - Rol del usuario al momento de la acción (snapshot)
 * @param {string}  [opts.ip_address]        - IP del cliente
 */
const logAudit = async ({ modulo, accion, registro_afectado, detalle, usuario_id, usuario_rol, ip_address }) => {
  try {
    await pool.query(
      `INSERT INTO audit_logs
         (modulo, accion, registro_afectado, detalle, usuario_id, usuario_rol, ip_address, fecha)
       VALUES (?, ?, ?, ?, ?, ?, ?, NOW())`,
      [
        modulo,
        accion,
