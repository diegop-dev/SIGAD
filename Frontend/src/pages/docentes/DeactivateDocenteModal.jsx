...
export const DeactivateDocenteModal = ({ docente, onClose, onSuccess }) => {
  ...
  const API_BASE = import.meta.env.VITE_API_URL 
    ...
    : 'http://localhost:3000';
  const profileImageUrl = docente.foto_perfil_url 
    ? `${API_BASE}${docente.foto_perfil_url}` 
    : null;
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden border border-slate-100">
        <div className="flex justify-between items-center px-6 py-5 border-b border-red-100 bg-red-50/50">
          <div className="flex items-center text-red-600">
            ...
            <h3 className="text-lg font-black tracking-tight">Confirmar baja del docente</h3>
          </div>
          <button 
            onClick={onClose} 
        ...
        <div className="p-6">
          <p className="text-slate-600 text-sm font-medium leading-relaxed mb-6">
            Estás a punto de dar de baja al siguiente docente:
          </p>
        ...
        <div className="bg-slate-50/50 px-6 py-5 border-t border-slate-100 flex justify-end gap-3">
          ...
          >
            {isSubmitting ? (
              ...
              <>
                <Trash2 className="w-4 h-4 mr-2" /> Dar de baja docente
              </>
            )}
          </button>
        </div>
